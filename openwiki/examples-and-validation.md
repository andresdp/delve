...
The repository includes `examples/product_reviews.json` and `examples/customer_support.txt` as direct corpus inputs. `examples/campus-bike/`, `examples/das-p1-2023/`, and `examples/pharmacy-food/` contain architecture-decision JSON inputs, per-example YAML configs, taxonomy text, graph PNGs, and timestamped taxonomy/document/message/cluster artifacts. The root `output/` similarly contains timestamped `architecture-decisions` and `reviews` result families. Treat all timestamped JSON/PNG files as generated samples, not authoritative input or tests. `EXAMPLES.md` describes a product-review run and an architecture-decision run.
