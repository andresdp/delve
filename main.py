...
Usage:
    ...
    python main.py --project-name MY_PROJECT --org-id MY_KEY --days 3
...
def parse_args() -> argparse.Namespace:
    ...
    input_group.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a corpus file (.txt or .json). If provided, skips LangSmith retrieval.",
    )
    input_group.add_argument(
        "--project-name",
        type=str,
        default="",
        help="LangSmith project name (only used when --corpus is not provided).",
    )
    input_group.add_argument(
        "--org-id",
        type=str,
        default="",
        help="LangSmith API key / org ID (only used when --corpus is not provided).",
    )
    input_group.add_argument(
        "--days",
        type=int,
        default=3,
        help="Number of days to look back for LangSmith runs (default: 3).",
    )
...
def _display_taxonomy(clusters: list) -> None:
...
def _display_documents(documents: list) -> None:
    ...
    for doc in display_docs:
        ...
        table.add_row(
            label or "N/A",
            content_preview + "..." if len(content_preview) >= 100 else content_preview,
        )
...
async def run(args: argparse.Namespace) -> None:
    ...
    if args.corpus:
        ...
        if not args.project_name or not args.org_id:
            logger.error("Either --corpus or both --project-name and --org-id must be provided")
            console.print("[bold red]❌ Error: Either --corpus or both --project-name and --org-id must be provided.[/bold red]")
            sys.exit(1)
        logger.info("Using LangSmith retrieval mode — project: %s, days: %d", args.project_name, args.days)
        invoke_input = {
    ...
    config = {}
    if args.model or args.fast_model:
        configurable = {}
        if args.model:
    ...
    model_display = args.model or default_config.model
    fast_model_display = args.fast_model or default_config.fast_llm
    console.print(Panel(
        f"[bold]Starting taxonomy generation pipeline...[/bold]\n\n"
        f"[dim]Model:[/dim] [cyan]{model_display}[/cyan]\n"
        f"[dim]Fast LLM:[/dim] [cyan]{fast_model_display}[/cyan]",
    ...
    if not args.quiet:
        try:
            ...
            graph_dir = Path(args.output) if args.output else Path("output")
